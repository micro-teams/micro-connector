// Package auth implements the machine half of enrolment: this machine asks the control plane to
// start an authorization, shows whoever is standing there a link to open, and polls until it is
// approved — at which point the control plane hands back a durable credential representing this
// device. How anyone is actually authenticated on that link is entirely the control plane's
// business; the machine only starts, waits, and keeps the result.
//
// The paths come from the brand (EnrollBase + "/start" and "/poll"). The field names below are the
// shape MicroTeams' control plane speaks; a product whose backend differs implements these two
// exchanges rather than reshaping them, because two spellings of the same handshake is how a shared
// component starts becoming three.
package auth
