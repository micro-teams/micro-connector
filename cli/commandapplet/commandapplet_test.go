...
// A registered command runs its handler, which reaches the backend through connector.http and
...
func TestAppletDefinesAndRunsCommand(t *testing.T) {
	...
	source := `
	  connector.command({
	    ...
	    run: (ctx) => {
	      const res = connector.http({ method: 'POST', path: '/chat/1/messages', body: { content: ctx.flags['text'] } })
	      if (res.status >= 400) throw new Error('failed ' + res.status)
	      connector.print('sent:' + res.body.content + ' by:' + res.body.senderId)
	    },
	  })
	`
...
func TestRequiredFlagEnforced(t *testing.T) {
	a, err := Load(`connector.command({name:'x', flags:[{name:'text',type:'string',required:true}], run:()=>{ throw new Error('should not run') }})`,
...
func TestFsSandbox(t *testing.T) {
	root := t.TempDir()
	a, err := Load(`
	  connector.command({ name: 'w', run: () => {
	    connector.fs.write('notes/a.txt', 'hi')
	    connector.print(connector.fs.read('notes/a.txt'))
	    connector.print(String(connector.fs.exists('../escape.txt')))
	    connector.fs.write('../escape.txt', 'nope')   // must NOT escape root
	  }})
...
func TestExec(t *testing.T) {
	var out bytes.Buffer
	a, err := Load(`
	  connector.command({ name: 'e', run: () => {
	    const r = connector.exec('printf', ['ok'])
	    connector.print('code:' + r.code + ' out:' + r.stdout)
	  }})
	`, Options{Out: &out})
	if err != nil {
		t.Fatal(err)
