// Package config is the single on-disk identity of this installation: where the control plane is,
// and the long-lived credential that represents this machine to it.
//
// Everything on the machine reads this one file — the long-running host and any ad-hoc command —
// so a machine is configured once and every part of it agrees. The file carries no notion of what
...
// durable credential minted for this device at enrolment. WS is an optional explicit
// control-channel URL — when empty it is derived from Base.
type Config struct {
